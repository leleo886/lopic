    ...
    "paths": {
        ...
        "/api/images/upload": {
            "post": {
                ...
                "description": "上传图片文件，@WebSocket /ws/upload \"上传进度通知：连接此WebSocket端点获取实时上传进度\" @WebSocketEvent upload_start \"上传开始：{upload_id, file_name, file_size}\" @WebSocketEvent upload_progress \"上传进度：{upload_id, file_name, progress, read, total}\" @WebSocketEvent upload_complete \"上传完成：{upload_id, file_name, file_url, thumbnail_url, image_id}\" @WebSocketEvent upload_error \"上传错误：{upload_id, file_name, error}\"",
                "consumes": [
                    "multipart/form-data"
                ],
                "produces": [
